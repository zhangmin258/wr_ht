...
func (j *JPush) buildMessage() {
	...
	if j.platform == Platform_ANDROID {
		...
		j.notice.SetAndroidNotice(&jpushclient.AndroidNotice{Alert: j.alert, Title: j.title, Extras: j.extras})
	} else if j.platform == Platform_IOS {
		j.pf.Add(Platform_IOS)
		j.notice.SetIOSNotice(&jpushclient.IOSNotice{Alert: j.alert, Badge: j.badge, Extras: j.extras})
...
//发送消息
func (j *JPush) PushMessageWithExtra(extarData map[string]interface{}, title string, alert string, users ...string) {
	j.alert = alert
	j.title = title
	j.users = users
	j.extras = extarData
